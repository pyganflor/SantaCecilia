...
class RecalcularCurvas extends Command
{
    ...
    public function handle()
    {
        ...
        $ciclos = DB::table('proyeccion_modulo_semana as p')
            ->join('variedad as v', 'v.id_variedad', '=', 'p.id_variedad')
            ->join('planta as pt', 'pt.id_planta', '=', 'v.id_planta')
            ->select('p.modelo')->distinct()
            ->where('p.estado', 1)
            ->where('v.estado', 1)
            ->where('pt.estado', 1)
            ->where('pt.tiene_ciclos', 1)
            ->where('p.tabla', 'C')
            ->where('p.semana', $semana_pasada->codigo)
            ->where('p.cosechados', '>', 0)
            ->get();
        foreach ($ciclos as $pos_c => $c) {
            dump($pos_c . '/' . count($ciclos));
            ...
            if ($ciclo->activo == 1 && $num_sem >= $ciclo->semana_poda_siembra - 3 && $ciclo->no_recalcular_curva == 0) {   // esta activo y es una semana minima 2 antes del inicio de cosecha
                ...
                if ($num_sem < $ciclo->semana_poda_siembra) {   // se trata de una semana antes del inicio de cosecha
                    $cosechado = DB::table('desglose_recepcion as dr')
                        ...
                        ->where('r.estado', 1)
                        ->where('r.fecha_ingreso', '<=', $semana_pasada->fecha_final . ' 23:59:59')
                    ...
                    dump($semana_pasada->fecha_final, opDiasFecha('+', 35, $ciclo->fecha_inicio), $cosechado, $getTallosProyectados, $porc_cosechado, $configuracion->proy_minimo_cosecha);
        ...
        $ciclos = DB::table('proyeccion_modulo_semana as p')
            ...
            ->where('pt.tiene_ciclos', 1)
            ->where('p.tabla', 'C')
            ->where('p.semana', $semana_actual->codigo)
            ->where('p.cosechados', '>', 0)
            ->get();
        foreach ($ciclos as $pos_c => $c) {
            dump($pos_c . '/' . count($ciclos));
            ...
            if ($ciclo->activo == 1 && $num_sem >= $ciclo->semana_poda_siembra - 2 && $ciclo->no_recalcular_curva == 0) {   // esta activo y es una semana minima 2 antes del inicio de cosecha
                ...
                if ($num_sem < $ciclo->semana_poda_siembra) {   // se trata de una semana antes del inicio de cosecha
                    $cosechado = DB::table('desglose_recepcion as dr')
                        ...
                        ->where('dr.id_modulo', $modulo->id_modulo)
                        ->where('r.estado', 1)
                        ->where('r.fecha_ingreso', '<=', $semana_actual->fecha_final . ' 23:59:59')
                        ->where('r.fecha_ingreso', '>=', opDiasFecha('+', 35, $ciclo->fecha_inicio))
                        ->get()[0]->cant;
                    $porc_cosechado = $getTallosProyectados > 0 ? intval(($cosechado * 100) / $getTallosProyectados) : 0;
                    if ($porc_cosechado >= $configuracion->proy_minimo_cosecha) {   // hay que mover una semana antes la curva
