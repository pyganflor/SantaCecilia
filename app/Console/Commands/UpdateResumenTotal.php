...
class UpdateResumenTotal extends Command
{
    ...
    public function handle()
    {
        ...
        if ($desde_par <= $hasta_par) {
            ...
            foreach (ConfiguracionEmpresa::All() as $finca) {
                ...
                foreach ($array_semanas as $sem) {
                    ...
                    $pedidos = Pedido::where('estado', 1)
                        ...
                        ->where('id_configuracion_empresa', $id_finca)
                        ->get();
                    $valor = 0;
                    ...
                    /* ----------------------------- regalias ------------------------- */
                    $costos_semanal = ResumenCostosSemanal::All()
                        ->where('id_empresa', $id_finca)
                        ->where('codigo_semana', $sem)
                        ->first();
