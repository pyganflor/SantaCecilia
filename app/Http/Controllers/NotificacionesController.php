...
class NotificacionesController extends Controller
{
    ...
    public function buscar_notificaciones(Request $request)
    {
        ...
        if (count($news) > 0) {
            foreach ($news as $n) {
                ...
                $n->url = "'" . $n->url . "'";
                $n->texto = "'" . $n->texto . "'";
                if ($n->notificacion->automatica == 1) {
                    $li .= '<li title="' . $n->texto . '">
                    ...
                    $li .= '<li title="' . $n->titulo . '" id="li_user_notification_' . $n->id_user_notification . '">
                            <div class="row" style="padding: 5px">
                            <div class="col-md-10">
                            <a href="javascript:void(0)" class="text-color_yura" onclick="alerta(' . $n->texto . ')">
                            <i class="fa fa-' . $n->notificacion->icono->nombre . '"></i> ' . $n->titulo . '
                            </a>
                            </div>
                            <div class="col-md-2 text-right">
                            <button type="button" class="btn btn-xs btn-yura_dark" onclick="delete_user_notification(' . $n->id_user_notification . ')">
    ...
    {
        $model = UserNotification::find($request->id);
        $model->delete();
        return [
            'mensaje' => '',
            'success' => true,
