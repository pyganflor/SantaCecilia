...
class CamasCiclosController extends Controller
{
    ...
    public function listar_camas(Request $request)
    {
        ...
        $camas = Cama::All();
        if ($finca_actual != 'T')
            $camas = $camas->where('id_empresa', $finca_actual);
        $camas = $camas->sortBy('area_trabajo')->sortBy('nombre');
        ...
        if (!$valida->fails()) {
            if (count(Cama::All()->where('nombre', '=', str_limit(mb_strtoupper(espacios($request->nombre)), 25))
                ...
                if ($model->save()) {
                    ...
                    /* ============== Actualizar finca en cicloActual ============== */
                    $cicloActual = $model->ciclo_actual();
                    if ($cicloActual != '') {
                        $cicloActual->id_empresa = $model->id_empresa;
                        $cicloActual->save();
                    }
                } else {
                    $success = false;
    ...
    public function listar_ciclos(Request $request)
    {
        ...
        if ($request->activo == 0) {
            ...
            $all_camas = Cama::All()->where('area_trabajo', 'PLANTAS MADRES');
            if ($finca_actual != 'T')
                $all_camas = $all_camas->where('id_empresa', $finca_actual);
            $all_camas = $all_camas->sortBy('area_trabajo')->sortBy('nombre');
            $camas = [];
            ...
            $ciclos = CicloCama::where('id_variedad', $request->variedad)
                ->where('activo', 1);
            if ($finca_actual != 'T')
                $ciclos = $ciclos->where('id_empresa', $finca_actual);
            $ciclos = $ciclos->get();
            $json = [
                'ciclos' => $ciclos,
                'variedades' => getVariedades()
    ...
    public function store_ciclo(Request $request)
    {
        if ($request->fecha_fin >= $request->fecha_inicio) {
            ...
            $ciclo->activo = 1;
            $ciclo->id_empresa = Cama::find($request->cama)->id_empresa;
    ...
    public function update_ciclo(Request $request)
    {
        if ($request->fecha_fin >= $request->fecha_inicio) {
            ...
            $ciclo->id_empresa = $ciclo->cama->id_empresa;
