    ...
    public function getDataBySemana($semana, $variedad, $desde, $opcion, $detalle)
    {
        ...
        if ($semana != '') {
            ...
            $cosecha = DB::table('desglose_recepcion as dr')
                ...
                ->where('c.fecha_ingreso', '>=', $semana->fecha_inicial)
                ->where('c.fecha_ingreso', '<=', $semana->fecha_final)
                ->get()[0]->cantidad;
            ...
            if ($ciclo_ini != '') { // esa semana inició un ciclo
                $data = [
                    ...
                    'info' => $ciclo_ini->poda_siembra . '-' . ($ciclo_ini->num_poda_siembra != '' ? $ciclo_ini->num_poda_siembra : $this->getPodaSiembraByCiclo($ciclo_ini->id_ciclo)),
                    'cosechado' => $cosecha,
