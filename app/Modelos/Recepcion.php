...
class Recepcion extends Model
{
    ...
    public function cantidad_tallosByFinca($finca)
    {
        $r = 0;
        foreach ($this->desgloses as $item) {
            if ($item->id_empresa == $finca) {
                $r += $item->cantidad_mallas * $item->tallos_x_malla;
            }
        }
        return $r;
    ...
    public function tallos_x_variedadByFinca($variedad, $finca)
    {
        $r = 0;
        foreach ($this->desgloses as $item) {
            if ($item->id_variedad == $variedad && $item->id_empresa == $finca) {
                $r += $item->cantidad_mallas * $item->tallos_x_malla;
            }
        }
        return $r;
    ...
    public function total_x_variedadByFinca($variedad, $finca)
    {
        $r = 0;
        foreach ($this->desgloses as $item) {
            if ($item->id_variedad == $variedad)
