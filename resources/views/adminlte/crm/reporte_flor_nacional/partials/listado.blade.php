@if (count($listado) > 0)
                ...
                <div style="max-height: 700px; overflow-x: scroll; overflow-y: scroll">
                    <table class="table-bordered"
                                ...
                                <tr onmouseover="$(this).addClass('bg-yura_dark')"
                                    ...
                                    <th class="padding_lateral_5 columna_fija_left_0 bg-yura_dark"
                                        ...
                                        <a href="javascript:void(0)" style="color: white"
                                            ...
                                            onclick="seleccionar_variedad('{{ $item['variedad']->id_variedad }}')">
                                    ...
                                    @php
                                        ...
                                        $listado[$pos_i]['porcentaje'] = porcentaje($total_nacional_var, $total_nacional, 1);
                                        $listado[$pos_i]['porcentaje_nacional'] = porcentaje($total_nacional_var, $item['cosechados'], 1);
                                    @endphp
                                    <th class="text-center columna_fija_right_0 bg-yura_dark"
                                        style="border-color: #9d9d9d">
                                        {{ number_format($total_nacional_var) }}
                                    </th>
                                    <th class="text-center bg-yura_dark" style="border-color: #9d9d9d">
                                        {{ porcentaje($total_nacional_var, $total_nacional, 1) }}%
                                    </th>
                                    <th class="text-center bg-yura_dark" style="border-color: #9d9d9d">
                                        {{ number_format($item['cosechados'] - $total_nacional_var) }}
                                    </th>
                                    <th class="text-center bg-yura_dark" style="border-color: #9d9d9d">
                                        {{ number_format($item['cosechados']) }}
                                    </th>
                                    <th class="text-center bg-yura_dark" style="border-color: #9d9d9d">
                                        {{ porcentaje($total_nacional_var, $item['cosechados'], 1) }}%
                                    </th>
                                </tr>
                            @endforeach
                        </tbody>
                        <tr class="tr_fijo_bottom_0">
                            ...
                            <th class="text-center th_yura_green" style="border-color: #9d9d9d">
                                {{ porcentaje($total_nacional, $total_procesado, 1) }}%
                            </th>
                        </tr>
                    </table>
                </div>
    ...
    <script>
        ...
        function seleccionar_check(pos_c) {
            ...
            if (activados > 10) {
                alerta(
                    '<div class="alert alert-warning text-center"><h3>Seleccione hasta un <b>MAXIMO de 10 VARIEDADES</b></h3></div>'
                );
                $('#check_var_' + pos_c).prop('checked', false);
            }
        }
        ...
        function generar_grafica_tallos() {
            ...
            @foreach ($listado as $pos => $item)
                if ($('#check_var_' + pos).prop('checked') == true) {
                    data_list = [];
                    ...
                    datasets.push({
                        label: '{{ $item['variedad']->nombre }}' + ': ' + '{{ $item['porcentaje_nacional'] }}%',
                        data: data_list,
        ...
        function generar_grafica_porcentaje_nacional() {
            ...
            @foreach ($listado as $pos => $item)
                if ($('#check_var_' + pos).prop('checked') == true) {
                    labels.push("{{ $item['variedad']->nombre }}: {{ $item['porcentaje_nacional'] }}%");
