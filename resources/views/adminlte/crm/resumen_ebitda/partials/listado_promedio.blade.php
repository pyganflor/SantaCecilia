        ...
        <tr id="tr_fija_bottom_0">
            ...
            @foreach($totales as $pos_t => $val)
                <th class="text-center bg-yura_dark" style="padding-left: 5px; padding-right: 5px">
                    {{$val['positivos'] > 0 ? number_format($val['cantidad'] / $val['positivos'], $number_format) : 0}}
                </th>
            @endforeach
        </tr>
    </table>
    ...
    function select_desglose_planta(id_pta, pos) {
        ...
        $.get('{{url('resumen_ebitda/select_desglose_planta')}}', datos, function (retorno) {
            ...
            for (i = 0; i < retorno.listado.length; i++) {
                ...
                row.innerHTML = '<th style="background-color: #e9ecef; border-color: #9d9d9d; padding-left: 5px; border-right: 2px solid">' +
