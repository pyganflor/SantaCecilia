<script>
    ...
    function listar_motivos_nacional() {
        get_jquery('{{ url('clasificaciones/listar_motivos_nacional') }}', {}, function(retorno) {
            $('#tab_motivos_nacional').html(retorno);
        });
    }
    ...
    function store_motivo_nacional() {
        datos = {
            _token: '{{ csrf_token() }}',
            nombre: $('#new_nombre_motivo').val(),
        };
        if (datos['nombre'] != '') {
            post_jquery_m('{{ url('clasificaciones/store_motivo_nacional') }}', datos, function() {
                listar_motivos_nacional();
            });
        }
    }
    ...
    function update_motivo_nacional(id) {
        datos = {
            _token: '{{ csrf_token() }}',
            id: id,
            nombre: $('#edit_nombre_motivo_' + id).val(),
        };
        if (datos['nombre'] != '')
            post_jquery_m('{{ url('clasificaciones/update_motivo_nacional') }}', datos, function() {
            });
    }
    ...
    function cambiar_estado_motivo_nacional(id) {
        datos = {
            _token: '{{ csrf_token() }}',
            id: id,
        };
        post_jquery_m('{{ url('clasificaciones/cambiar_estado_motivo_nacional') }}', datos, function() {
            listar_motivos_nacional();
        });
    }
