...
@foreach ($listado as $item)
    ...
    <tr>
        ...
        <td class="text-center" style="border-color: #9d9d9d">
            <input type="number" style="width: 100%;" class="text-center" value="{{ $item->tallos_x_ramo }}"
                id="edit_tallos_x_ramo_{{ $item->id_inventario_frio }}">
        </td>
        <td class="text-center" style="border-color: #9d9d9d">
            <input type="text" readonly style="width: 100%; background-color: #e9ecef" class="text-center"
        ...
        <td class="text-center" style="border-color: #9d9d9d; width: 60px">
            <div class="btn-group">
                <button type="button" class="btn btn-xs btn-yura_dark" title="Modificar"
                    onclick="update_inventario('{{ $item->id_inventario_frio }}', '{{ $planta }}', 'N')">
                    <i class="fa fa-fw fa-pencil"></i>
                </button>
                <button type="button" class="btn btn-xs btn-yura_warning" title="Botar"
                    onclick="botar_inventario('{{ $item->id_inventario_frio }}', '{{ $planta }}', 'N')">
                    <i class="fa fa-fw fa-trash"></i>
                </button>
                <button type="button" class="btn btn-xs btn-yura_danger" title="Eliminar Registro"
                    onclick="delete_inventario('{{ $item->id_inventario_frio }}', '{{ $planta }}', 'N')">
...
<script>
    $('#btn_nacional_planta_{{ $planta }}').html(
        '{{ $total_planta }} <i class="fa fa-fw fa-eye hidden" id="icon_nacional_planta_{{ $planta }}"></i>'
    );
</script>
