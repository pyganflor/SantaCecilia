<div class="box box-success">
    ...
    <div class="box-body">
        ...
        <div class="input-group" style="margin-top: 10px">
            ...
            <select id="id_empresa" class="form-control input-yura_default">
                @foreach(getFincasPropias() as $f)
                    <option value="{{$f->id_empresa}}" {{(isset($cama) && $f->id_empresa == $cama->id_empresa) ? 'selected' : ($f->id_empresa == getUsuario(Session::get('id_usuario'))->finca_activa ? 'selected' : '')}}>
                        {{$f->empresa->nombre}}
                    </option>
                @endforeach
            </select>
