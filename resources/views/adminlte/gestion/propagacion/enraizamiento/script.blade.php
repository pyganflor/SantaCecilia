<script>
    $('#vista_actual').val('enraizamiento');
    var cant_form_add = 1;
    ...
    function listar_enraizamientos() {
        ...
        $.LoadingOverlay('show');
        $.get('{{url('enraizamiento/listar_enraizamientos')}}', datos, function (retorno) {
            $('#listado_siembras').html(retorno);
    ...
    function add_row_form_add() {
        ...
        var select_form_add_contenedor = $('#form_add_contenedor_1').html();
        $('#table_form_add').append('<tr id="row_form_add_' + cant_form_add + '">' +
            ...
            '</td>' +
            '<td class="text-center div_cargar_semanas_' + cant_form_add + '">' +
            '<select name="form_add_contenedor_' + cant_form_add + '" id="form_add_contenedor_' + cant_form_add + '" style="width: 100%">' +
            select_form_add_contenedor +
            '</select>' +
            '</td>' +
            '<td class="text-center">' +
    ...
    function buscar_enraizamiento_semanal(num_row) {
        ...
        if (datos['variedad'] > 0) {
            ...
            $.post('{{url('enraizamiento/buscar_enraizamiento_semanal')}}', datos, function (retorno) {
                $('#form_add_contenedor_' + num_row).val(retorno.bandeja);
    ...
    function store_enraizamiento() {
        ...
        for (i = 1; i <= cant_form_add; i++) {
            var d = {
                ...
                contenedor: $('#form_add_contenedor_' + i).val(),
            };
        ...
        post_jquery('{{url('enraizamiento/store_enraizamiento')}}', datos, function () {
            ...
            $('#form_add_planta_1').val('');
            $('#form_add_variedad_1').val('');
            select_planta($('#form_add_planta_1').val(), 'form_add_variedad_1', 'div_cargar_variedades_1', '<option value=>Seleccione</option>', 0)
        });
    }
</script>