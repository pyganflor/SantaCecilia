...
<table style="width: 100%">
    ...
    <tr>
        <td class="text-center" style="border-color: #9d9d9d">
            <input type="date" id="fecha_inicio_new" class="form-control text-center" style="width: 100%" value="{{date('Y-m-d')}}" required>
        </td>
        <td class="text-center" style="border-color: #9d9d9d">
            <select id="poda_siembra_new" class="form-control" style="width: 100%">
        ...
        <td class="text-center" style="border-color: #9d9d9d">
            <input type="number" id="plantas_muertas_new" class="form-control text-center" style="width: 100%" value="0" required>
        </td>
        <td class="text-center" style="border-color: #9d9d9d">
            <input type="number" id="conteo_new" class="form-control text-center" style="width: 100%" value="0" required>
        </td>
    </tr>
</table>
        ...
        if (datos['variedad'] != '' && datos['modulo'] != '' && datos['area'] != '' && datos['variedad'] != '' && datos['fecha_inicio'] != '' && datos['poda_siembra'] != '')
            modal_quest('modal-quest_store_ciclo', '<div class="alert alert-info text-center">¿Está seguro de <strong>CREAR y EMPEZAR</strong> este ciclo?</div>',
                '<i class="fa fa-fw fa-exclamation-triangle"></i> Confirmar acción', true, false, '{{isPC() ? '35%' : ''}}', function () {
                    post_jquery('{{url('sectores_modulos/store_crear_activar_modulo')}}', datos, function () {
